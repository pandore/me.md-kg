...
export async function query(question: string) {
  ...
  const entityResults: Array<{ id: string; name: string; type: string; summary: string | null }> = [];
  for (const kw of keywords) {
    const matches = db.prepare(
      'SELECT id, name, type, summary FROM entity WHERE LOWER(name) LIKE ?'
    ).all(`%${kw}%`) as Array<{ id: string; name: string; type: string; summary: string | null }>;
  ...
  return {
    ...
    data: {
      ...
      entities: entityResults.slice(0, 10).map(e => ({ name: e.name, type: e.type, summary: e.summary })),
